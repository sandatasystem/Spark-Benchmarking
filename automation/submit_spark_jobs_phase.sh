...
function poll_till_benchmark_completes(){
    ...
    while true; do
        ...
        benchmark_output=$(kubectl logs $spark_operator_pod -n $namespace 2>&1)
        container_not_created=$(kubectl logs $spark_operator_pod -c spark-kubernetes-driver -n $namespace 2>&1 | grep -o 'container "spark-kubernetes-driver" in pod')
        ...
        benchmark=$(kubectl logs $spark_operator_pod -c spark-kubernetes-driver -n $namespace | grep -Po "Total time is (\d*)" | grep -oP "(\d*)")
...
function submit_join_job(){
	...
	do
            echo "RUNNING A $join_type JOIN ON 2 DATAFRAMES WITH SIZE $row_size\n\n"
...
function submit_teragen_terasort_teravalidate(){
    ...
    teragen_rows=(1g 2g 2k 2m)
    for row_size in "${teragen_rows[@]}"
    do
        # TERAGEN
	export DF_ROWS_ARGUMENTS="$row_size"
        printf "PERFORMING TERAGEN ON $row_size ROWS"
	j2 $HOME/Spark-Benchmarking/yamls/spark-benchmark-teragen2.yaml > /tmp/spark-benchmark-teragen.yaml
	kubectl apply -f /tmp/spark-benchmark-teragen.yaml -n $namespace

        poll_till_benchmark_completes "spark-benchmark-teragen-driver"
        benchmark=$(kubectl logs spark-benchmark-teragen-driver -c spark-kubernetes-driver -n $namespace | grep -Po "Total time is (\d*)" | grep -oP "(\d*)")
	printf "\n\n\n FINISHED TERAGEN SRIPT. TIME TO GENERATE $row_size rows is $benchmark milliseconds\n\n\n"
	...
	j2 $HOME/Spark-Benchmarking/yamls/spark-benchmark-terasort2.yaml > /tmp/spark-benchmark-terasort.yaml
	kubectl apply -f /tmp/spark-benchmark-terasort.yaml -n $namespace 

        poll_till_benchmark_completes "spark-benchmark-terasort-driver"
        benchmark=$(kubectl logs spark-benchmark-terasort-driver -c spark-kubernetes-driver -n $namespace | grep -Po "Total time is (\d*)" | grep -oP "(\d*)")
	printf "FINISHED TERASORT SRIPT. TIME TO SORT $row_size of data is $benchmark milliseconds"
